const Modal = require("../models/columns");
...
const getModalsByTable = async (req, res) => {
  try {
    const { tableId } = req.params; // Lấy tableId từ query parameters
    const modals = await Modal.findAll({
      where: {
        tableId, // Lọc theo tableId
      },
      raw: true,
    });
    const formattedModals = modals.map((modal) => ({
